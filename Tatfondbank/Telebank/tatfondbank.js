...
//	"absURL": "http://algea/ubs_asp/esm/esm_get_data_bss.asp",
	...
	var TRUE_DEBUG = true;
/*
При выборе регистрации в системе по номеру кредитного договора, клиенту необходимо ввести 14 цифр кредитного договора, затем система 
формирует запрос поиска введенного номера кредитного договора в АБС.
В случае удачного завершения поиска, система предлагает клиенту ввести 4 цифры серии паспорта, затем 6 цифр номера паспорта.
Клиент вводит паспортные данные, далее система формирует запрос поиска введенных данных в АБС.
В случае успешного завершения поиска, регистрация "личного кабинета" клиента прошла успешно.
В противном случае, система АБС возвращает ошибку. Система автоинформирует клиента. Проигрывается сообщение: "Вы ввели неверные данные". 
Таким же сообщением система оповещает и в первом случае, если клиент ввел неверный номер договора.
*/

OutputDebugString("ICC test");
...
function RegisterByCreditContract() {
	var attempts = constants.attempts;
	while( attempts-- ) {
		if ( attempts != constants.attempts-1) {
			 PlayTFB("RepeatAttempt");
		}
		var contractID = PlayAndCollectTFB("CompleteRegister",constants.creditContractIDLength, constants.enterContractIDTimeout);	
		if ( contractID.length < constants.creditContractIDLength ) {
			continue;
		}
...
function TFBConsultant() {
	...
	if (IsMobileNumber( phone )  ){
		//TRUE_DEBUG var info = db.getUserInfoByPhone( phone );
